...
#             yield Request(baseurl + product, callback=self.parse_product)
    ...
    def parse(self, response):
        ...
        for jsobj in jsobjs:
            ...
            for devs, deals in data['devices'].items():
                for term, deal in deals['boxes'].items():
                    ...
                    price = price_lable.css(
                        '.price:not(.flat.obsolete)::attr(data-price)'
                    ).get('')
        ...
        if not jsobjs:
            for listing in response.css(
                '.box-content > .csc-default.skin-mb-l'
            ).xpath('parent::div[@class="box-content"]'):
                product = listing.css('p > a::text').get()
                price = listing.css('.price::text').get()
                currency = price[:re.search(r'\d', price).start()]
                if not currency: # Assume currency is mentioned after price figure
                    currency = price[-re.search(r'\d', price[::-1]).start():]
                price = ''.join(re.findall('[0-9][0-9 .,]+', price))
